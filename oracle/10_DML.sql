/*
   ...
   : 데이터 조작 언어로 테이블에 값을 삽입(INSERT)하거나,
     ...
     3) INSERT INTO 테이블명 (서브쿼리);
       ...
       - 서브쿼리에 결과값이 INSERT 문에 지정된 테이블 컬럼의 개수와
...
/*
   INSERT ALL
   두 개 이상으 테이블에 각각 INSERT 하는데 동일한 서브 쿼리가 사용되는 경우
...
/*
   ...
   SET 컬럼명 = 변경하려는값,
    ...
    - SET 절에서 여려 개의 컬럼을 콤마(,)로 나열해서 값을 동시에 변경할 수 있다.
    - WHERE절을 생략하면 모든 형의 데이터가 변경된다.
...
-- EMP_SALARY 테이블에서 선동일 사원의 급여을 7000원으로,
-- 보너스를 0.2로 변경
UPDATE EMP_SALARY
SET SALARY = 7000, BONUS = 0.2
WHERE EMP_NAME = '선동일';

-- 모든 사원의 급여를 기준 급여에서 10프로 인상한 급여(기존급여 * 1.1)로 변경
...
-- EMP_SALARY 테이블에서 아시아 지역에 근무하는 직원들의
-- 보너스를 0.3으로 변경
SELECT EMP_ID
FROM EMP_SALARY, DEPT_COPY D, LOCATION L
...
/*
   ...
   - WHERE 절을 제시하지 ㅇ낳으면 전체 행이 삭제
*/
SELECT * FROM EMP_SALARY;
-- EMP_SALARY 테이블에서 선동일 사장의 데이터를 지우기
DELETE FORM EMP_SALARY WHERE EMP_ID = 200;
