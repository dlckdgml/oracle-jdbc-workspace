 /*
    ...
    - 관계형(Relational) 데이터베이스
     ...
     3. 데이터의 중복을 최소화할 수 있으며 사용하기 편리하고 데이터의 무결성,
        트랜잭션 처리 등 데이터베이스 관리 시스템으로 뛰어난 성능을 보여준다.
 ...
 */
 ...
 -- EMPLOYEE 테이블의 전체 사원들의 사번(EMP_ID), 이름(EMP_ID), 급여(SALARY)만을 조회
 ...
 */
 ...
 -- 오늘날짜 : SYSDATE
 -- CEIL(NUMBER) : 매개값으로 전달되는 수를 올림하는 함수(소수점을 올림)
 ...
 /*
    ...
    - 별칭을 부여할 때 띄어쓰기 혹은 특수문자가 별칭에 포함될 경우 반드시 큰따옴표("")로 감싸줘야한다.
 */
 ...
 */
 ...
 -- EMPLOYEE 테이블에서 사번, 직업명, 급여, 단위(원) 조회
 SELECT 
   EMP_ID AS 사번,
   EMP_NAME AS 직원명,
   SALARY AS 급여,
 ...
 /*
    연결 연산자 : ||
    - 여러 컬럼값들을 마치 하나의 컬럼인것처럼 연결하거나, 컬럼값과 리터럴을 연결할 수 있으ㅡㅁ
 */
 ...
 SELECT EMP_NAME || '의 월급은 ' || SALARY || '원입니다.' AS "급여 정보"
 FROM EMPLOYEE;
...
WHERE MANAGER_ID IS NULL AND DEPT_CODE IS NULL;
 ...
 FROM EMPLOYEE
 WHERE (SALARY + (BONUS * SALARY)) * 12 IS NULL AND EMP_NAME LIKE '%하%';
 ...
 /*
    ...
    - ASC : 오름차순 정렬 (생략시 기본값)
    - DESC : 내림차순 정렬
    - NULLS FIRST : 정렬하고자 하는 컬럼값에 NULL이 있을 경우 해당 데이터를 맨 앞에 배치
                    (생략시 DESC일때 기본값)
