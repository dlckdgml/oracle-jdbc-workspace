...
-- 사원의 수, 보너스를 받는 사원의 수, 총 급여, 평균급여, 최저 급여, 최고 급여 조회
...
SELECT 
 ...
 FROM EMPLOYEE
 GROUP BY DEPT_CODE;
 ORDER BY DEPY_CODE NULLS FIRST;
...
/*
  HAVING
  - 그룹에 대한 조건을 제시할 때 사ㅛㅇ하는 구문
    ( 주로 그룹 함수한 결과를 가지고 비교 수행)
    
    SELECT문 실행순서
    SELECT | 조회하고자 하는 컬럼명  as 별칭 | 계산식 | 함수식
    FROM   조회하고자 하는 테이블명
    WHERE  조건식
    GROUNPP BY 그릅기준에 해당하는 컬럼 | 계산식 | 함수식
    HAVING 
    ODER BY
*/

-- 부서별 평군 급여가 300만원 이상인 직원의 평균 급여을 조회
-- (부서코드, 평균굽여)
  ...
  /*
     ...
     - 여러 개의 쿼리문에서 조회하려고 하는 컬럼의 개수와 이름이 같아야
       ...
       UNION ALL(합집합) : 두 쿼리문을 수행한 결과값을 하나로 합쳐서 추출
                          (중복되는 행을 제거하지 않음)
       INTERSECT(교집합) : 두 쿼리문을 수행한 결과값에 중된 결과값만 추출
       MINUS(차집합) : 선행 쿼리문의 결과값에서 후행 쿼리문의 결과값을
                      뺀 나머지 결과값만 추출
  */

-- 부서 코드가 D5인 사원들의 사번, 사원명, 부서코드, 급여을 조회
...
-- 1. UNION
-- 2. 부서 코드가 D5인 사원 또는 급여가 300만원 초과인 사원들의 사번, 사원명,
--    부서 코드, 급여를 조회
  SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
  FROM EMPLOYEE
  WHERE DEPT_CODE = 'D5'
